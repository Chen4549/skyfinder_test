...
from resnet import resnet50, resnet50_fusion
from loss import *
...
parser.add_argument('--bucket_start', type=int, default=0, choices=[0, 3, 2],
...
parser.add_argument('--dataset', type=str, default='skyfinder_30_balanced', choices=['skyfinder', 'agedb', 'skyfinder_30', 'skyfinder_30_final', 'skyfinder_30_balanced'], help='dataset name')
...
parser.add_argument('--pretrained', type=str, default='/gpfs/data/shenlab/hc4549/ucla/imbalanced-regression/pretrained/resnet50-19c8e357.pth', help='checkpoint file path to load backbone weights')
parser.add_argument('--evaluate', action='store_true', help='evaluate only flag')

parser.add_argument('--fusion', type=bool, default=False, help='append metadata into resnet regressor')
parser.add_argument('--feature_extract', type=bool, default=False, help='extract feature from resnet')
...
def main():
    ...
    if args.fusion:
        model = resnet50_fusion(fds=args.fds, bucket_num=args.bucket_num, bucket_start=args.bucket_start,
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        if args.fusion:
            train_loss = train_fusion(train_loader, model, optimizer, epoch)
        else:
            train_loss = train(train_loader, model, optimizer, epoch)

        if args.fusion:
            val_loss_mse, val_loss_l1, val_loss_gmean = validate_fusion(val_loader, model, train_labels=train_labels)
        else:
...
def train_fusion(train_loader, model, optimizer, epoch):
    ...
    for idx, (inputs, targets, weights, meta) in enumerate(train_loader):
        data_time.update(time.time() - end)
        inputs, targets, weights, meta= \
            inputs.cuda(non_blocking=True), targets.cuda(non_blocking=True), weights.cuda(non_blocking=True), meta.cuda(non_blocking=True)
        if args.fds:
            # outputs, _ = model(inputs, targets, epoch)
            outputs, _ = model(inputs, meta, targets, epoch)
        else:
            # outputs = model(inputs, targets, epoch)
            outputs = model(inputs, meta, targets, epoch)
    ...
    if args.fds and epoch >= args.start_update:
        ...
        encodings, labels = [], []
        with torch.no_grad():
            for (inputs, targets, _, meta) in tqdm(train_loader):
                # inputs = inputs.cuda(non_blocking=True)
                inputs, meta = inputs.cuda(non_blocking=True), meta.cuda(non_blocking=True)
                # outputs, feature = model(inputs, targets, epoch)
                outputs, feature = model(inputs, meta, targets, epoch)
        ...
        # print(f"DEBUG: encodings shape: {encodings.shape}")
...
def train(train_loader, model, optimizer, epoch):
    ...
    for idx, (inputs, targets, weights, _) in enumerate(train_loader):
        data_time.update(time.time() - end)
...
def validate_fusion(val_loader, model, train_labels=None, prefix='Val'):
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, targets, _, meta) in enumerate(val_loader):
            inputs, targets, meta = inputs.cuda(non_blocking=True), targets.cuda(non_blocking=True), meta.cuda(non_blocking=True)
...
def validate(val_loader, model, train_labels=None, prefix='Val'):
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, targets, _, _) in enumerate(val_loader):
...
# def extract_visual_features(loader, model):
#     model.eval()
#     features = []
#     labels = []
    
#     print("===> Extracting 2048-d visual features...")
#     with torch.no_grad():
#         for idx, (inputs, targets, _, _) in enumerate(tqdm(loader)):
#             inputs = inputs.cuda(non_blocking=True)
            
#             # Use the updated forward pass to get the encoding
#             output, encoding = model(inputs) 
...
def extract_visual_features_with_mapping(loader, model):
    ...
    with torch.no_grad():
        for idx, (inputs, targets, _, meta) in enumerate(tqdm(loader)):
            inputs = inputs.cuda(non_blocking=True)
            # Use the forward pass to get the encoding
            # Note: Ensure you use the correct model (ResNet vs ResNet_fusion)
            output, encoding = model(inputs) 
            
            features.append(encoding.cpu().numpy())
            # Get paths from the dataset inside the loader
            # This assumes your AgeDB __getitem__ or the dataframe is accessible
