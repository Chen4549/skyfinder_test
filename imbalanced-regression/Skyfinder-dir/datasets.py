...
class AgeDB(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = transform(img)
        label = np.asarray([row['label']]).astype('float32')
        weight = np.asarray([self.weights[index]]).astype('float32') if self.weights is not None else np.asarray([np.float32(1.)])

        return img, label, weight

        # path = row['path'] 
        # return img, label, weight, path
    ...
    def _prepare_weights(self, reweight, max_target=121, lds=False, lds_kernel='gaussian', lds_ks=5, lds_sigma=2):
        ...
        for label in labels:
            ...
            # bin_idx = min(max_target - 1, int(label * 10))
            # value_dict[bin_idx] += 1
        ...
        weights = [np.float32(1 / (x + 1e-9)) if x == 0 else np.float32(1 / x) for x in num_per_label]
