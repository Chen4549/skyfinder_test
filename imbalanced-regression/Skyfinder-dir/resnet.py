...
class ResNet_fusion(nn.Module):
    ...
    def __init__(self, block, layers, fds, bucket_num, bucket_start, start_update, start_smooth,
        ...
        self.linear = nn.Linear(feature_dim + 16, 1)
        # ------------------------------
        ...
        if fds:
            self.FDS = FDS(
                ...
                feature_dim=512 * block.expansion + 16, bucket_num=bucket_num, bucket_start=bucket_start,
    ...
    def forward(self, x, meta, targets=None, epoch=None):
        # print("WRONGGGGGGGGGGGGGG")
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        # --- NEW: Process Metadata and Concatenate ---
        meta_features = self.metadata_mlp(meta)
        combined_features = torch.cat((encoding, meta_features), dim=1)
        # ----------------------------------------------

        # encoding_s = encoding
        encoding_s = combined_features
...
def resnet50_fusion(**kwargs):
    return ResNet_fusion(Bottleneck, [3, 4, 6, 3], **kwargs)
